...
1. `README.md`：项目简介、当前能力和文档导航。
2. `docs/ARCHITECTURE.md`：当前 runtime 架构、模块映射和稳定边界。
3. `docs/PROGRESS.md`：durable status、未清债务、候选顺序和阶段索引。
...
- `openspec/specs/`：全部当前长期 capability specs 的规范入口。
...
- 优先跑确定性验证：相关 `pytest`/Ruff，最终运行 `python -I scripts/verify.py`；PowerShell 可使用
  `scripts/verify.ps1` 薄包装。
- 只在所属 durable facts 变化时更新 `docs/PROGRESS.md`；tracked HANDOFF 不回填当前 branch、HEAD、
  worktree、candidate、merge/push 或 remote parity。
...
PowerShell host 可使用 `scripts/verify.ps1` 薄包装；required checks 仍由同一个 Python 入口定义。
