...
`harden-grounded-citation-instruction` remediation 已归档、合并并推送到 `main`，当前 eval
change 已恢复。由于 runtime 已变化，旧 review、live result 与 attestation 证据全部失效。

Paused exception：更新后的真实 DeepSeek run 在 commit `3dfd06d` 上完成 8 calls，但
grounded-text 仍因 citation framing 与 prompt injection 失败。当前 change 再次冻结 evaluator
与 runtime 修改，等待独立 grounded evidence framing remediation。
