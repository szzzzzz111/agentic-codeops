...
V17 Verification Runner 已归档到 `openspec/changes/archive/2026-06-03-v17-verification-runner/`。下一阶段开始前必须先创建新的 OpenSpec change，并同步本文件和 `.harness/review_checklist.md`。
...
- 当前无 active stage；不得继续扩展 V17 runtime，V18 前不得自动在 patch apply 后运行验证，不得根据验证失败自动生成 patch，不得持久化 verification result，不得创建 worktree，不得自动 commit/push。
