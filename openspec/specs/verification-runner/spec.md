...
V17 初始白名单 SHALL 只包含 `pytest`、`ruff` 和 `verify` 三个稳定标签。`verify` SHALL 映射到 `powershell -ExecutionPolicy Bypass -File scripts/verify.ps1`。系统 MAY 支持中英文触发词，但 MUST 在执行前归一化为白名单标签。
