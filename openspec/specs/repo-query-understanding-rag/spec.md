...
记录 V8 已实现的检索前理解和非向量化 repo-local lexical RAG：系统在仓库检索前生成确定性 `SearchPlan`，对允许访问的 repo 文本文件生成轻量 chunk，使用关键词、符号、路径和文件名信号排序，并返回带相对路径和行号的 citation。该能力不接 embedding provider、向量库、LLM rewrite、rerank 或 memory。
