...
默认 fake Patch Authoring provider MUST 保持离线确定性，并且 MUST NOT 生成真实 diff。显式配置的 OpenAI-compatible provider MAY 返回结构化 JSON unified diff，但系统 MUST 在创建 pending patch 前校验 schema、citation、路径和 diff。
...
#### Scenario: 合法结构化 diff 创建 pending patch

- **WHEN** 显式配置 provider 返回合法结构化 diff
