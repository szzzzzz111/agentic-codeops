...
The CLI MUST distinguish implemented command packaging from runtime capability changes. It MUST call existing service/agent boundaries and MUST NOT claim or implement behavior outside those boundaries.
...
- **AND** it prints a safe summary of the returned `trace_id`, `answer`, `related_files`, and `tool_calls`
...
- **THEN** the command MUST request a patch proposal only
